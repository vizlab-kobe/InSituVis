...
int Program::exec( int argc, char** argv )
{
  ...
  kvs::ShaderSource::AddSearchPath("../../../../ParticleBasedRendering/Lib");
    ...
    kvs::ColorImage image = proc.render( input, timer, volumes, particle_time, generation_time, composition_time, projection_time, createbuffer_time, readback_time, averaging_time, num_particle);
    
    //world.reduce( 0, render_time, render_time, MPI_MAX );
    world.reduce( 0, projection_time, projection_time, MPI_MAX );
    world.reduce( 0, createbuffer_time, createbuffer_time, MPI_MAX );
    ...
    world.reduce( 0, generation_time, generation_time, MPI_MAX );
    world.reduce( 0, composition_time, composition_time, MPI_MAX );
    world.reduce( 0, num_particle, num_particle, MPI_MAX);
    render_time = projection_time + readback_time + averaging_time;
    if ( rank == 0 )
    {
        std::cout << "Render time : " << render_time << " [sec]" << std::endl;
        std::cout << "Particle time : " << particle_time << " [sec]" << std::endl;
        std::cout << "Generation time : " << generation_time << " [sec]" << std::endl;
        std::cout << "Composition time : " << composition_time << " [sec]" << std::endl;
	std::cout << "number of particle : " << num_particle << std::endl;
	timer.start();
        image.write( "output.bmp" );
	timer.stop();
	std::cout << "output time : " << timer.sec() << " [sec]" <<std::endl;
