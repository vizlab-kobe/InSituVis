    ...
    const kvs::mpi::Communicator& comm )
{
    ...
    const std::string repeats = ToString( input.repetitions, 4 );
    return prefix + "_" + nprocs + "_" + width + "x" + height + "_" + repeats;
}
    ...
    const local::Process& proc )
{
    ...
    os << "min" << ","
       ...
       << min_times.mapping << ","
       << min_times.rendering << ","
       << min_times.rendering_creation << ","
       << min_times.rendering_projection << ","
       << min_times.rendering_ensemble << ","
       << min_times.readback << ","
       << min_times.transmission << ","
       << min_stats.nregions << ","
       << min_stats.ncells << ","
    ...
    os << "max" << ","
       ...
       << max_times.mapping << ","
       << max_times.rendering << ","
       << max_times.rendering_creation << ","
       << max_times.rendering_projection << ","
       << max_times.rendering_ensemble << ","
       << max_times.readback << ","
       << max_times.transmission << ","
       << max_stats.nregions << ","
       << max_stats.ncells << ","
...
int Program::exec( int argc, char** argv )
{
    ...
    // Output final image.
    if ( is_master ) { frame.colorImage().write( basename + "_image.bmp" ); }
