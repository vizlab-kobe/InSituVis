/*****************************************************************************/
/**
 *  @file   CameraPathControlledAdaptorMulti_mpi.h
...
#include "EntropyBasedCameraPathControllerMulti.h"
#include <list>
#include <queue>
...
class CameraPathControlledAdaptorMulti :
        public InSituVis::mpi::Adaptor,
        public InSituVis::EntropyBasedCameraPathControllerMulti
{
public:
    using BaseClass = InSituVis::mpi::Adaptor;
    using FrameBuffer = BaseClass::FrameBuffer;
    using Controller = InSituVis::EntropyBasedCameraPathControllerMulti;
...
private:
    ...
    kvs::Vec3ui m_viewDim{ 1, 1, 1 };

public:
    CameraPathControlledAdaptorMulti( const MPI_Comm world = MPI_COMM_WORLD, const int root = 0 ): BaseClass( world, root ) {}
    virtual ~CameraPathControlledAdaptorMulti() = default;
    ...
    const kvs::Vec3ui& viewPointDimention() const { return m_viewDim; }
...
private:
    ...
    std::vector<int> getMaximalLocations( const std::vector<Viewpoint::Location>& locations, const std::vector<float>& entropies );
...
#include "CameraPathControlledAdaptorMulti_mpi.hpp"
