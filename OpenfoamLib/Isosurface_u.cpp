...
void Isosurface_u( const std::vector<float> &values, int ncells, int nnodes, const std::vector<float> &vertex_coords, const std::vector<float> &cell_coords, const std::vector<int> &label, int time, float min_value, float max_value,   std::string stlpath, float cameraposx, float cameraposy, float cameraposz, const size_t repetitions,float isothr )
{
  ...
  CalculateMinMax( min_value, global_miny, global_minz, max_value, global_maxy, global_maxz );
  ...
  */
  ...
  volume->setVeclen( 1 );
  volume->setNumberOfNodes( nnodes - nonode_count );//nonode_count?
  ...
  rankname << std::setw(2) << std::setfill('0') << rank;
  std::string filerank = rankname.str();
  std::string mlname = "./output_volume" + filerank + ".kvsml";
  volume->write(mlname,true,false);
  ...
  for( size_t i = 0; i < repetitions; i++)
    {
      ...
      float e_time = 0;
      float g_time = 0;
      kvs::PolygonObject* poly_object2 = new kvs::Isosurface(volume,isothr,kvs::Isosurface::PolygonNormal);
      poly_object2->setOpacity( 255 );
      kvs::RGBColor color2 = tfunc.colorMap().at(isothr);
      poly_object2->setColor(color2);
      kvs::PolygonObject* poly_object3 = new kvs::Isosurface(volume,0.5,kvs::Isosurface::PolygonNormal);
      poly_object3->setOpacity( 255 );
      kvs::RGBColor color3 = tfunc.colorMap().at(0.5);
      poly_object3->setColor(color3);
      kvs::PolygonObject* poly_object4 = new kvs::Isosurface(volume,(max_value+min_value)/2,kvs::Isosurface::PolygonNormal);
      poly_object4->setOpacity( 255 );
      kvs::RGBColor color4 = tfunc.colorMap().at((max_value+min_value)/2);
  ...
  if( rank == 0 )
    {
      ...
      std::ostringstream ss,sa,sb,sc,sd;
      ss << std::setw(5) << std::setfill('0') << time;
      sa << std::setw(3) << std::setfill('0') << cameraposx;
      sb << std::setw(3) << std::setfill('0') << cameraposy;
      sc << std::setw(3) << std::setfill('0') << cameraposz;
      sd << std::setw(3) << std::setfill('0') << isothr;
      std::string num = ss.str();
      std::string numx = sa.str();
      std::string numy = sb.str();
      std::string numz = sc.str();
      std::string numthr = sd.str();
      std::string name = "./Output/output_result_mix_pbvr_u_" + num + "_" + numx + "_" + numy + "_" + numz + "_" + numthr + ".bmp";
      timer.start();
      image.write( name );
      timer.stop();
