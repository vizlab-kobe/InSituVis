...
extern "C" void Isosurface( double values[], int size, int dimx, int dimy, int dimz, double x_coords[], double y_coords[], double z_coords[], int time )
{
    int rank, nnodes;
    ...
    volume->setMinMaxExternalCoords( kvs::Vec3( local_minx, local_miny, local_minz ), kvs::Vec3( local_maxx, local_maxy, local_maxz ) );
  
    const kvs::PolygonObject::NormalType n = kvs::PolygonObject::VertexNormal;
    const bool d = false;
    kvs::TransferFunction t( 256 );
    t.setRange( volume );
    const double i = ( volume->maxValue() - volume->minValue() ) * 0.5 + volume->minValue();
    ...
    const kvs::Mat3 R = kvs::Mat3::RotationX( 20 ) * kvs::Mat3::RotationY( 20 );
	...
	std::string name = "./Output/output_result_isosurface_" +num +".bmp";
	image.write( name );
      }
}
